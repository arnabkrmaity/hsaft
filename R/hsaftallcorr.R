...
# Introduction of Correlation among proteins and groups all together
...
# add C++ sampling for beta


hsaftallcorr <- function(ct, X, method.tau = c("fixed", "truncatedCauchy","halfCauchy"), tau = 1,
                         ...
                         r, n.seq, pk)
{
  ...
  sigma_sq        <- Sigma2;
  bG              <- rep(0, r)
  sigma.bG.square <- rep(1, r)
  bP              <- rep(0, pk)
  BP              <- rep(0, p)
  sigma.bP.square <- rep(1, pk)
  B               <- rep(0, p)
  ...
  bGout         <- matrix(0, r, effsamp)
  bPout         <- matrix(0, pk, effsamp)
  ...
  for(i in 1:niter)
  {
    ...
    ## update beta ##
    # Update beta in block for each cancer
